...
namespace iTradex.UI.Pages.Investor
{
    public partial class SalesOrder : System.Web.UI.Page
    {
       ...
       // string accountReference = session.AccountNumber;
        ...
        private void SalesOrders()
        {
            try
            {
                GetSession session = new GetSession();
                CommonFunction cmDataTable = new CommonFunction();
                string showAddData = "select Instrument,TransactionType,ShareQuantity,Rate,Status,TransactionTime,TransactionDate from TradeOrderFromWeb where(InvestorACRef='" + session.AccountNumber + "' and TransactionType='S' and TransactionDate='" + DateTime.Today + "') order by TransactionTime, Instrument";

                DataTable dtSalesOrder = cmDataTable.GetDatatable(showAddData);
                ...
                rptSalesOrder.DataSource = dtSalesOrder;
                rptSalesOrder.DataBind();
            }
            catch (Exception ex)
            {
        ...
        protected void ddlInstrument_TextChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    //ClientScript.RegisterStartupScript(this.GetType(), "Alert", "<script type='text/javascript'>alert('Please Select Instrument');window.location='SalesOrder.aspx';</script>'");
                }
            }
            catch (Exception ex)
            {
